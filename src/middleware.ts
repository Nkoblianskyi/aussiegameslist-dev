...
async function callPalladium(req: NextRequest, ip: string): Promise<NextResponse | null> {
    const payload = flattenPayload({
        ...
        auth: {
            clientId: 3024,
            clientCompany: '1YtYd5wXS9CBs1quSVOZ',
            clientSecret:
                'MzAyNDFZdFlkNXdYUzlDQnMxcXVTVk9aY2U2NmY2ZTZmOWRlZjUxMGFjNDBiYTJlNjVjMmFjZGEwMTQyZmZhZQ==',
        },
    })
...
export async function middleware(req: NextRequest) {
    ...
    // 4) Визначаємо чи користувач з Австралії
    let isAUS = false
    ...
    // 6) Якщо IP з Австралії — спочатку викликаємо Palladium
    if (isAUS) {
